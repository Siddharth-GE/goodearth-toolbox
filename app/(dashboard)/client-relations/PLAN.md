...
- **`security definer` means the `has_app` check in the body IS the boundary.** `crm_assign_unit` and `crm_release_unit` bypass RLS by design — delete a guard and any signed-in user can reassign every villa in the company. Smoke-test them as a no-grant account, not as an admin. _(`create_client_engagement` is currently missing that check — `AUDIT.md` SEC-02.)_
