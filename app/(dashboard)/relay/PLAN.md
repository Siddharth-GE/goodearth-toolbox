...
- **Never rebuild `pusher_chain_state` from an older migration.** It has been defined in **six** files (`0036`, `0038`, `0039`, `0041`/`0042`, `0043`, `0064`) and a `create view` is a full replacement, not a patch. `0041` was written from `0036`'s copy and silently dropped the department columns `0038` had added, breaking All trails against the live database until `0042`. Always start from the live definition: `select pg_get_viewdef('pusher_chain_state'::regclass, true);` **It has two consumers outside Relay** — Client Relations and Reporter — so a seventh definition must check both, and re-check the grants: `drop view` + `create view` resurrects Supabase's default write privileges every time.
...
- **Work not filed under a stage has no honest place on a wave**, so it is counted out loud beside it ("3 not on the wave") rather than dropped. A calm-looking wave with work missing from it is the flattering number this tool exists to remove.
