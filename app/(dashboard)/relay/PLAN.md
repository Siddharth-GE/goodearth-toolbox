...
- **Never rebuild `pusher_chain_state` from an older migration.** It has been defined in **five** files (`0036`, `0038`, `0039`, `0041`/`0042`, `0043`) and a `create view` is a full replacement, not a patch. `0041` was written from `0036`'s copy and silently dropped the department columns `0038` had added, breaking All trails against the live database until `0042`. Always start from the live definition: `select pg_get_viewdef('pusher_chain_state'::regclass, true);` **It now has two consumers outside Relay** — Client Relations and Reporter — so a sixth definition must check both.
...
- **A queued trail must never show a timer.** A `TimerDial` reading "0 of 4 days" looks like a clock that has started, which is precisely what the queue exists to avoid. Queued rows read "Waiting" / "not started".
...
- **The seams.** `pusher_chain_links` surfaced both ways, Google Chat notifications (fire-and-forget — never block a write on them), and `getRelayPulse()` grown into what the collated Dashboard reads.
