...
count counts things, not labels** (decision 9 below). **Stage 6 — the money —
shipped alone on 2026-08-12**: `0055` applied (seven quals widened, verified
by the migration's own assertion), `po_lines`/`bills`/`budget_report_lines`
registered, derived values (`FieldDef.derive` → `DERIVED` in `derive.ts`),
the `vendors` lookup, the Spend-vs-budget starter, and the amber
`grantWarning` beside the Reporter checkbox in Settings. **Stage 7 shipped
2026-08-13**: `0056` applied (the two CRM fact views — owner views, WHERE +
column list as the gate, prose columns asserted absent), `crm_milestones` +
`crm_receipts` datasets (two, never one join), the `crm_balance_due` derived
field and the Sales & collections starter. **Stage 8 shipped 2026-08-13**:
the five remaining datasets (`goods_receipt_lines`, `stock`,
`selection_lines`, `relay_chains`, `units`) — no migration needed, every
source already readable by all authenticated — plus `DatasetDef.pageOrder`
