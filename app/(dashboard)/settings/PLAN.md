...
- **`bill_approval_cap(uid)` answers about the caller only**, since `0071`, applied 2026-08-17. It used to answer about any user id, to any signed-in caller, and `profiles` is readable by everyone — so every colleague's approval ceiling was one request away. It **raises** for anyone else's id unless the caller is an admin — not returns null, because null in that function means _unlimited_ and a refusal must never read as one. `can_approve_bills` and `can_approve_indents` are the same shape and return `false`. Settings reads the `roles` table for what it shows, not these functions, so nothing on its screens changed.
