...
| Migrations    | `0001`–`0054`, all applied. Nothing pending.                                                                                                                 |
...
| **Reporter**          | Production | Was the Dashboard stub; stages 1–5 of 10 merged 2026-08-12. A report builder over a dataset registry — columns, dropdown-only filters (founder rule), two-level grouping, measures, sort, subtotals — rendering as a composed page: KPI band, chart (6 forms via `recharts` behind `components/ui/chart/*`), table, and a CSV download of every matched line (`lib/csv.ts`, now shared with Selections). Reports save by name (`0054`); starting points are code constants, copyable, never editable. A distinct count counts by internal id, never by a name that repeats. One dataset so far (`indent_lines`), so one starting point. Next: the money (Stage 6) — the widest permission change the app has made, and it ships alone. Plan: its `PLAN.md`. |
