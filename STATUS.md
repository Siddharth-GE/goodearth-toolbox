...
| **Reporter**          | Production | Was the Dashboard stub; renamed by `0052`, stages 1–2 of 10 merged 2026-08-12. A report builder over a dataset registry: columns, dropdown-only filters (founder rule), two-level grouping, measures, sort, subtotals and grand totals — validated through `parseReportSpec`, computed in tested pure TypeScript. One dataset so far (`indent_lines`). Next: charts (Stage 3). Plan: its `PLAN.md`. |
