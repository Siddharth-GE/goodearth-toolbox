...
| Migrations    | `0001`–`0052`, all applied. Nothing pending.                                                                                                                 |
...
| **Reporter**          | In build   | Was the Dashboard stub; renamed by `0052`. Stages 1–2 of 10 built on `feature/reporter` (pending browser sign-off): a report builder over a dataset registry — columns, filters, two-level grouping, measures, sort, subtotals and grand totals, all validated through `parseReportSpec` and computed in tested pure TypeScript. One dataset so far (`indent_lines`). Plan: its `PLAN.md`.   |
