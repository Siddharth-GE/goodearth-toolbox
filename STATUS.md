...
| **Reporter**          | Production | Was the Dashboard stub; stages 1–4 of 10 merged 2026-08-12. A report builder over a dataset registry — columns, dropdown-only filters (founder rule), two-level grouping, measures, sort, subtotals — rendering as a composed page: KPI band, chart (6 forms via `recharts` behind `components/ui/chart/*`), table, and a CSV download of every matched line (`lib/csv.ts`, now shared with Selections). A distinct count counts by internal id, never by a name that repeats. One dataset so far (`indent_lines`). Next: saved reports (Stage 5). Plan: its `PLAN.md`. |
