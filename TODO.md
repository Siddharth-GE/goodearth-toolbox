...
- **The estimator becomes the construction backbone** (founder, 2026-08-19; the staged plan is in `plan.md` on the feature branch). Step A shipped as `0076`: a material links to the catalogue item it is bought as. Step B shipped as `0077`: Submit locks one official estimate per villa (EST/…, frozen costs, submitter recorded — the old "locked estimates" and "reference numbers" follow-ups, done). Step D shipped as `0079`: POs raisable without an indent for bulk/urgent buys (no ceiling on direct lines, by design). Still to come, in order: indents pull from the official estimate (Step C, in review — construction budget lines retire as the source in Step F, after C soaks); store issues record which work they serve; then a Supervisors app and over-issue warnings. **Data task for Masters once 0076 is live: enter the construction raw materials as `kind='material'` items and link each estimator material to its item — unlinked materials can't feed requests.**
