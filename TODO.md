...
- **The estimator becomes the construction backbone** (founder, 2026-08-19; the staged plan is in `plan.md` on the feature branch). Step A shipped as `0076`: a material links to the catalogue item it is bought as. Step B shipped as `0077`: Submit locks one official estimate per villa (EST/…, frozen costs, submitter recorded — the old "locked estimates" and "reference numbers" follow-ups, done). Step C shipped as `0078`: indents pull from the official estimate through `estimate_takeoff_facts` (the construction-plan pull retires in Step F, after C has soaked). Step D shipped as `0079`: POs raisable without an indent for bulk/urgent buys (no ceiling on direct lines, by design). Step E shipped as `0080`: a plot issue names the work it serves, and the official estimate shows issued-vs-estimated per work. Founder feedback 2026-08-20, all shipped: direct-to-site deliveries name their work at receipt (`0081` — they never pass a store, so it would slip) and count in the comparison; the indent stage picker is retired in favour of the one works vocabulary. The units master shipped as `0082`: one Masters list (Masters → Units) behind every unit picker and validation; the estimator's private list and procurement's hard-coded eight are retired. Step F shipped 2026-08-20: the construction-plan pull is retired (pointer screen, schema and history untouched), and material outside the official estimate is a permanent reconciliation entry needing an estimator's approval (`0083` — founder's soak correction). Open founder question: do the Budgets → Construction screens retire entirely now that they feed nothing, or stay as the QS team's reference? Next: Phase 2 — a Supervisors app (request-for-issue per work) and over-issue warnings to the estimate's submitter (the arithmetic is `lib/estimator/compare.ts`, already flagging on screen). **Data task for Masters once 0076 is live: enter the construction raw materials as `kind='material'` items and link each estimator material to its item — unlinked materials can't feed requests.**
- **Estimator follow-ups** — the tool itself is built (`0074`, 2026-08-19: materials, mixes, work recipes, estimates, live costing). Still open: **estimate against actual**, comparing an estimate to the POs and bills raised for that villa — the reason it was built on the shared works list rather than a private one; **the reporting surface the founder sketched on 2026-08-19** — an estimate PDF (render `groupLineCosts` + the takeoff through `lib/pdf/`), a per-work rate-analysis print, and material/labour usage filters across estimates (all screens over the existing per-line numbers, no new arithmetic — see the Estimator PLAN.md). **Scheduling was cut by the founder** on 2026-08-19 and is not pending: the workbook's "Shedule & Progress track" sheet has no home, and Relay stores no per-activity dates by its own decision.
