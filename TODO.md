...
- **The estimator becomes the construction backbone** (founder, 2026-08-19; the staged plan is in `plan.md` on the feature branch). Step A shipped as `0076`: a material links to the catalogue item it is bought as. Still to come, in order: Submit locks an official estimate per villa (folds in the "locked estimates" and "reference numbers" follow-ups below); indents pull from the official estimate (construction budget lines retire as the source); POs raisable without an indent for bulk/urgent buys; store issues record which work they serve; then a Supervisors app and over-issue warnings. **Data task for Masters once 0076 is live: enter the construction raw materials as `kind='material'` items and link each estimator material to its item — unlinked materials can't feed requests.**
- **Estimator follow-ups** — the tool itself is built (`0074`, 2026-08-19: materials, mixes, work recipes, estimates, live costing). Still open: **locked estimates** (freeze one at today's rates — additive, a snapshot table or rate columns on the line); **reference numbers** if the team ever wants `EST/SAA/001` (the mint-in-SQL pattern from Indents); **estimate against actual**, comparing an estimate to the POs and bills raised for that villa — the reason it was built on the shared works list rather than a private one; **the reporting surface the founder sketched on 2026-08-19** — an estimate PDF (render `groupLineCosts` + the takeoff through `lib/pdf/`), a per-work rate-analysis print, and material/labour usage filters across estimates (all screens over the existing per-line numbers, no new arithmetic — see the Estimator PLAN.md). **Scheduling was cut by the founder** on 2026-08-19 and is not pending: the workbook's "Shedule & Progress track" sheet has no home, and Relay stores no per-activity dates by its own decision.
