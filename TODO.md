...
2. **Production promotion, after the vet**: `npm run db:apply -- --project pajfrgnkapicdgangjey --commit` (applies `0084`, `0085`, `0086` in order) → `npm run db:types` → `npm run db:compare` must report no differences → merge `staging → master` → press one real write button on production (record a labour log).
