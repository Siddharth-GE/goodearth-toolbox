 ...
 * basket and one commit bar — shared by the Construction tree and the
 * Indents direct-add, exactly the experience the Selections picker
 * established (DESIGN.md's "build the third copy into a shared
 * component"; Selections keeps its own copy for now, with its
 * space-chips and request-item extras, and migrates here later).
 *
 ...
 * indent) is the caller's business. The basket lives entirely in the
 * browser and nothing is written until onCommit — pressing + costs
 * nothing.
 */
      ...
      <DialogContent className="flex h-[88vh] max-w-6xl flex-col gap-3">
        ...
        <div className="border-border bg-surface flex flex-wrap items-center justify-between gap-3 rounded-xl border px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={commit} disabled={saving || distinctItems === 0}>
              {saving ? "Adding…" : "Add"}
            </Button>
          </div>
        </div>
