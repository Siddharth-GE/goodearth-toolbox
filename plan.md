...
- **Phase 4 — Space linking.** 🔨 In progress 2026-09-02 — Fable planned it and drafted `0094`; Opus and Sonnet build. _(Detailed below.)_ Migration `google_chat_spaces` (staging first: `npm run db:apply -- --project <ref> --commit`); `ADDED_TO_SPACE` auto-match + announcement; `/link` dialog.
...
- [ ] `[Fable]` `0094` drafted, applied to staging, types regenerated, committed.
- [ ] `[Opus]` `events.ts` growth + tests, `space-match.ts` + tests, `spaces.ts`, route wiring, `SECURITY.md` line.
- [x] `[Sonnet]` `cards.ts` growth + tests.
- [ ] `[Fable]` review; builders commit under their own names; push; CI; PR into `staging`.
- [ ] `[Founder]` "Opens a dialog" ticked on `/link`; staging vet (a)–(c).
