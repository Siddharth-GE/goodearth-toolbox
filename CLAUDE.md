...
Everything in it is a `SELECT`: **no tool's code writes another tool's table.** Three documented exceptions — `indent_approvers`/`bill_approvers` are Settings-owned despite living in Indents'/Bills' migrations; the `projects_seed_schedule` trigger (`0045`) lets creating a project in Masters seed Relay's schedule, declared by _Relay's_ migration so the coupling points the right way; and Client Relations writes Masters (`0050`, `0051`) through two column-narrow definer functions plus the `units_seed_engagement` trigger. **A cross-tool trigger or definer function not listed there is what nobody finds until it misfires.**
