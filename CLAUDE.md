...
references nothing). Client Relations also reads the shared
`units`/`plots`/`projects`/`clients`/`profiles`. Masters, `profiles` and
`items` are shared, not another tool's. Business Planning also _writes_ its
own `business_plan_targets` on save; Reporter reads only the
`business_plan_target_facts` view, never the engine.
