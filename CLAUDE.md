...
7. Merge `staging` → `master` only after browser testing and sign-off. Then **confirm a Production deployment exists for that exact commit** — `gh api repos/<owner>/<repo>/deployments` filtered to `environment == "Production"`, and its SHA must equal `git rev-parse --short origin/master`. **A merge is not a deployment**, and on 2026-08-17 nine commits sat merged, green and undeployed for hours because the identical SHA had already been built as a staging preview (`BUGCATCHER.md` #12). Press one real write button on production afterwards.
