...
- **Pausing.** The free tier also pauses a project after **7 days without a request**, and between releases nobody uses production — so it happened (idle from 2026-08-22, found paused 2026-09-03, restored 2026-09-05 with the ledger, every policy and all 237 auth settings intact). A paused project answers the management API with `status: INACTIVE`, its database host stops resolving, and the login page still renders — "the site is up" proves nothing. The fix in place since 2026-09-05 (the founder's choice over the Pro plan): **a weekly Vercel cron** (`vercel.json`) calls `/api/keep-alive` on the production deployment every Monday at 09:00 IST, and the route reads one row through the admin client. It needs `CRON_SECRET` in Vercel's Production environment — without it the route answers 503 and the cron log goes red, which is the point. **Paste the value with no space before or after it**: Vercel refuses the entire production deployment — instantly, before building — when `CRON_SECRET` carries leading or trailing whitespace, and the only place the reason shows is that deployment's own log (2026-09-05; the merge sat undeployed until the value was re-entered). A failed production deploy leaves the previous build serving, so nothing breaks for staff, but nothing new ships either. Crons run **only on the production deployment**, so staging never pings and never needs the variable; a paused staging is the same restore. Before any production step, read `GET /v1/projects/pajfrgnkapicdgangjey` and expect `ACTIVE_HEALTHY`; if not, `POST /v1/projects/pajfrgnkapicdgangjey/restore` with the same token and poll — about four minutes. Check the cron's runs in Vercel → Settings → Cron Jobs after each Monday until trusted.
